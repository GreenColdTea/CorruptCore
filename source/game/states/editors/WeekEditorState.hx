...
class WeekEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function addWeekUI() {
		...
		hideCheckbox.onClick = () -> weekFile.hideStoryMode = hideCheckbox.checked;
	...
	function addOtherUI() {
		...
		hiddenUntilUnlockCheckbox.onClick = () -> weekFile.hiddenUntilUnlocked = hiddenUntilUnlockCheckbox.checked;
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Mods.modsImages('menubackgrounds/menu_' + assetName)) || #end
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Mods.modsImages('storymenu/' + assetName)) || #end
	...
	private static function onSaveError(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_file = null;
		FlxG.log.error("Problems with saving file");
	}
}
...
class WeekEditorFreeplayState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function addFreeplayUI() {
		...
		var copyColor:PsychUIButton = new PsychUIButton(10, bgColorStepperR.y + 25, "Copy Color", function() {
		...
		hideFreeplayCheckbox.onClick = () -> weekFile.hideFreeplay = hideFreeplayCheckbox.checked;
