	...
	/*override function updateNote(beat:Float, daNote:Note, pos:Vector3, player:Int)
	{
		if (daNote.isSustainNote)
		{
			...
			if (shitGotHit)
			{
				...
				if (revPerc >= 0.5) // DownScroll logic
				{
			...
			if (Conductor.songPosition > PlayState.instance.noteKillOffset + daNote.strumTime)
			{
				if (daNote.mustPress && !PlayState.instance.cpuControlled && !daNote.ignoreNote && !PlayState.instance.endingSong && (daNote.tooLate || !daNote.wasGoodHit)) {
					PlayState.instance.noteMiss(daNote);
				}
